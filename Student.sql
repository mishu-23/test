...
--select count(*) from prieteni p1, prieteni p2 where p1.ID_STUDENT1=p2.ID_STUDENT2 and p1.ID_STUDENT2=p2.ID_STUDENT1;
  ...
  ALTER TABLE note ADD CONSTRAINT uq_student_materie UNIQUE (id_student, id_curs);
  select * from note;
  insert into note values (16401, 1, 1, 5, '13-FEB-30', '13-FEB-30', '13-FEB-30');
  delete from note where id = 16401;
  ALTER TABLE note DROP CONSTRAINT uq_student_materie;
  
  
  
  SELECT COUNT(*) AS numar_prieteni_simetrici
  FROM prieteni p1
  JOIN prieteni p2 ON p1.id_student1 = p2.id_student2 AND p1.id_student2 = p2.id_student1;
